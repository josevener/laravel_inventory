...
class ProductSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if ($warehouses->isEmpty()) {
            $this->command->error('No warehouses found! Seed warehouses first.');
            return;
        }
        ...
        foreach ($products as $p) {
            ...
            // Random realistic stock per warehouse
            foreach ($warehouses as $warehouse) {
                $stock = 0;
                if (rand(1, 100) > 25) { // 75% chance of having stock
                    $stock = rand(5, 600);
                    // Force some low stock items
                    if ($stock > $p['reorder'] && rand(1, 10) === 1) {
                        $stock = rand(1, $p['reorder'] - 1);
                    }
                }
                ...
                ProductWarehouse::create([
                    ...
                    'current_stock'  => $stock,
                    'total_value'    => $stock * $p['cost'],
                ]);
            }
        }

        $this->command->info('ProductSeeder completed! 21 products + stock in all warehouses.');
    }
}