...
export default function Create({ projects, nextNumber }) {
  ...
  const addProduct = (product) => {
    if (fields.some(f => f.product_id === product.id.toString())) {
  ...
  const onSubmit = (data) => {
    router.post(safeRoute("pull_out.store"), {
      ...data,
      nextNumber,
    }, {
      onSuccess: () => {
  ...
  return (
    <AuthenticatedLayout
      breadCrumbLink={safeRoute("pull_out.index")}
      breadCrumbLinkText="Pull Out"
      breadCrumbPage="New Pull Out"
    >
      <div className="w-full mx-auto">
        <div className="flex items-center gap-6">
          <Truck className="h-14 w-14 text-primary" />
          <div>
            <h1 className="text-4xl font-bold">Pull Out</h1>
