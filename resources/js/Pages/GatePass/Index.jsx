...
export default function GatepassIndex({ gatePasses, filters, projects, type }) {
  ...
  const printGatePass = (gpId) => {
    const routeName = isDispatch
      ? "gatepass.dispatch.print_gatepass"
      : "gatepass.pullout.print_pullout"

    const link = document.createElement("a")
    link.href = safeRoute(routeName, { gatepass: gpId })
    link.target = "_blank"
  ...
  return (
    <AuthenticatedLayout>
      <Head title={isDispatch ? "Dispatch Gate Passes" : "Pull Out"} />
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold">
                {isDispatch ? "Dispatch Gate Passes" : "Pull Out"}
              </h1>
              <p className="text-muted-foreground">
                Manage outgoing stock {isDispatch ? "dispatches" : "pull-outs"}
              </p>
            </div>
          </div>
          ...
          <Button asChild>
            <Link href={safeRoute(`gatepass.${type}.create`)}>
              ...
              New {isDispatch ? "Dispatch Gate Pass" : "Pull Out"}
            </Link>
          </Button>
        </div>
        ...
        {gatePasses.data.length > 0 ? (
            ...
            <Card>
              <CardContent className="p-0">
                <Table>
                  <TableHeader>
                    <TableRow>
                      {isDispatch && <TableHead>Gate Pass No</TableHead>}
                      <TableHead>Date</TableHead>
                      <TableHead>Project</TableHead>
                      <TableHead>Authorized Bearer</TableHead>
                  ...
                  <TableBody>
                    {gatePasses.data.map((gp) => (
                      <TableRow key={gp.id}>
                        ...
                        <TableCell>
                          {format(new Date(gp.created_at), "dd MMM yyyy, hh:mm a")}
                        </TableCell>
          ...
          <EmptyState
            ...
            description="You haven't created any gate passes yet."
            primaryAction={{
              label: "Create Gate Pass",
              onClick: () => router.visit(safeRoute(`gatepass.${type}.create`)),
            }}
          />
        )}
